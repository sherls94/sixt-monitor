...
export function ProviderResultCard({ r }: { r: ProviderResult }) {
  return (
    <Card className="p-4 flex items-center gap-3">
      <ProviderBadge provider={r.provider} size={44} />

      <div className="min-w-0 flex-1">
        <div className="font-medium text-foreground truncate">{r.provider}</div>
        {r.status === 'ok' ? (
      ...
      <div className="flex items-center gap-3 shrink-0">
        {r.price != null && (
          <div className="text-right">
            <div className="font-semibold text-foreground">${r.price.toFixed(2)}</div>
